...
% % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % % %
% ref(:)=dbs_nanzscore(ref(:)); %     Image1(:)=ea_nanzscore(Image1(:),'robust');
% ref=(ref+2.5)/5; % set max/min to -/+ 2.5 standard deviations
% ref(ref<0)=0; ref(ref>1)=1;
% src(:)=dbs_nanzscore(src(:)); %     Image1(:)=ea_nanzscore(Image1(:),'robust');
% src=(src+2.5)/5; % set max/min to -/+ 2.5 standard deviations
% src(src<0)=0; src(src>1)=1;

jim = cat(4,0.1*src+0.9*ref,0.4*src+0.6*ref,0.9*src+0.1*ref);
Img = cat(4,src,ref,jim);
